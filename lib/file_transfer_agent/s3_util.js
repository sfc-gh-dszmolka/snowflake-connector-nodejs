...
function S3Util(connectionConfig, s3, filestream) {
  ...
  const redactHeaders = (headers) => {
    ...
    const sensitiveKeys = ["x-amz-security-token", "authorization"];
  ...
  this.createClient = function (stageInfo, useAccelerateEndpoint) {
    ...
    // add a middlewarestack for sending the request/response logs to console 
    AwsS3.middlewareStack.add(
      (next, context) => async (args) => {
        console.log("[AWS-SDK REQUEST] ", {
        method: args.request?.method,
        hostname: args.request?.hostname,
        path: args.request?.path,
        headers: redactHeaders(args.request?.headers),
      });

      const result = await next(args);

      console.log("[AWS-SDK RESPONSE] ", {
    ...
    {
      step: "finalizeRequest", // runs before sending request
